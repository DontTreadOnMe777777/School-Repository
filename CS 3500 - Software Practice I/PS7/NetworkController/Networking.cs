    ...
    public static class Networking
    {
        ...
        public static TcpListener StartServer(Action<SocketState> toCall, int port)
        {
            ...
            try
            {
                // start a tcplistener
                listener.Start();
                //invoke the callback for accepting clients on a new thread
        ...
        {
            // extract the tuple from the AsyncResult
            ...
            try
            {
                ...
                SocketState socketstate = new SocketState(state.Item2, socket);
                // invoke the callback passed through the ar
            ...
            {
                // Create a new socket state with error flag/message set
                Socket tmpSocket = null;
                SocketState errorState = new SocketState(state.Item2, tmpSocket);
                errorState.ErrorOccured = true;
                errorState.ErrorMessage = "No valid IP or URL";

                // Invoke the callback with the new socketstate to send the error
        ...
        public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
        {
            ...
            try
            {
                ...
                if (!foundIPV4)
                {
                    // Declare a new SocketState with error message
                    Socket tmpSocket = null;
                    SocketState errorState = new SocketState(toCall, tmpSocket);
                    errorState.ErrorOccured = true;
            ...
            try
            {
                var result = socket.BeginConnect(ipAddress, port, ConnectedCallback, socketState);
                // Check that the socket connects to the server within 3 seconds, or throw an error
                result.AsyncWaitHandle.WaitOne(3000, true);
                if (!socket.Connected)
                {
                    socketState.ErrorOccured = true;
                    socketState.ErrorMessage = "Connection timed out!";
                    socketState.OnNetworkAction(socketState);
                    return;
                }
            }
            ...
            {
                socketState.ErrorOccured = true;
                socketState.ErrorMessage = "Error connecting to server!";
                socketState.OnNetworkAction(socketState);
                return;
            }
        }
        ...
        private static void ConnectedCallback(IAsyncResult ar)
        {
            ...
            catch
            {
                ...
                state.ErrorMessage = "Error completing the connection with the server!";
                state.OnNetworkAction(state);
                return;

            }

            // Invoke toCall Action with a new SocketState
        ...
        public static void GetData(SocketState state)
        {
            ...
            {
                state.ErrorOccured = true;
                state.ErrorMessage = "Error receiving data!";
                state.OnNetworkAction(state);
            }
        }
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                lock (state.data)
                {
                    ...
                    //If an empty message was received, throw an error
                    if (numBytes == 0)
                    {
                        state.ErrorOccured = true;
                        state.ErrorMessage = "No data received!";
                        state.OnNetworkAction(state);
                    }
                    // Get the message and append it to data in socketstate
        ...
        public static bool Send(Socket socket, string data)
        {
            try
            {
                ...
                {
                    // change message to bytes and send 
