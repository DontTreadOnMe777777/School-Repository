...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        //    Assert.IsTrue(testLocalSocketState.ErrorOccured);
        //    Assert.AreEqual("No IPV4 addresses found", testLocalSocketState.ErrorMessage);
        //}
        ...
        public void TestConnectInvalidHost()
        {
            ...
            Networking.ConnectToServer(saveClientState, "doesnotwork", 2112);

            NetworkTestHelper.WaitForOrTimeout(() => clientActionCalled, NetworkTestHelper.timeout);

            Assert.IsTrue(clientActionCalled);
            Assert.IsTrue(testLocalSocketState.ErrorOccured);
            Assert.AreEqual("No valid IP or URL", testLocalSocketState.ErrorMessage);
        }

        [TestMethod]
        public void TestFullConnectionStopServer()
        {
            ...
            TcpListener listener = Networking.StartServer(saveClientState, 2112);
            Networking.ConnectToServer(saveClientState, "localhost", 2112);
        ...
        public void TestConnectInvalidPort()
        {
            ...
            Networking.ConnectToServer(saveClientState, "localhost", -1);

            Assert.IsTrue(clientActionCalled);
            Assert.IsTrue(testLocalSocketState.ErrorOccured);
            Assert.AreEqual("Error connecting to server!", testLocalSocketState.ErrorMessage);
        }

        [TestMethod]
        public void TestGetDataInvalidSocketState()
        {
            bool clientActionCalled = false;
            ...
            SocketState state = new SocketState(saveClientState, null);
            Networking.GetData(state);

            Assert.IsTrue(clientActionCalled);
            Assert.IsTrue(testLocalSocketState.ErrorOccured);
            Assert.AreEqual("Error receiving data!", testLocalSocketState.ErrorMessage);
        }

        [TestMethod]
        public void TestGetSendNullData()
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
        ...
        [TestMethod]
        public void TestGetSendAndCloseNullData()
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
            testLocalSocketState.OnNetworkAction = x => { };
            Assert.IsFalse(Networking.SendAndClose(testLocalSocketState.TheSocket, null));
            Assert.IsFalse(testLocalSocketState.TheSocket.Connected);
        }

        [TestMethod]
        public void TestGetSendNullSocket()
        {
            Assert.IsFalse(Networking.Send(null, "epic"));
        }

        [TestMethod]
        public void TestGetSendAndCloseNullSocket ()
        {
            Assert.IsFalse(Networking.SendAndClose(null, "epic"));
        }
        ...
        [DataRow(false)]
        [DataTestMethod]
        public void TestSendAndCloseEmptyString(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
            ...
            Networking.SendAndClose(testLocalSocketState.TheSocket, "");
