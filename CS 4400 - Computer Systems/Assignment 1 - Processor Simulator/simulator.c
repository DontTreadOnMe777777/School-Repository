 ...
 *
 * Completed by: Brandon Walters, u1199080
*/
...
int main(int argc, char** argv)
{
  ...
  for(i = 0; i < NUM_REGS; i++)
    {
      // This is the %esp register, must be set to 1024 at start
      if(i == 6)
	{
	  registers[i] = 1024;
	}
...
instruction_t* decode_instructions(unsigned int* bytes, unsigned int num_instructions)
{
  ...
  for(i = 0; i < num_instructions; i++){
    // Pulls 32 bits out of the bytes array, equivalent to 1 instruction
    unsigned int bits = bytes[i];
    
    // Properly decodes the necessary instruction information using shifts and masks
    int16_t immediate = bits;
    unsigned char second_reg = (bits >> 17) & 0x1F;
    unsigned char first_reg = (bits >> 22) & 0x1F;
    unsigned char opcode = bits >> 27; 

    // Creates an instruction and properly assigns the decoded values to it
    instruction_t instr_to_add;
    instr_to_add.opcode = opcode;
    instr_to_add.first_register = first_reg;
    instr_to_add.second_register = second_reg;
    instr_to_add.immediate = immediate;

    // Adds the instruction to our return array
    retval[i] = instr_to_add;
  }
...
unsigned int execute_instruction(unsigned int program_counter, instruction_t* instructions, int* registers, unsigned char* memory)
{
  ...
  case cmpl:
    ...
    // Unsigned versions, used for the carry flag comparison
    uint64_t unsigned_first = (uint64_t)registers[instr.first_register];
    uint64_t unsigned_second = (uint64_t)registers[instr.second_register];
    ...
    // Carry flag
    if(unsigned_first > unsigned_second)
      {
	carryFlag = 1;
      }
    // Overflow flag
    ...
    // Add all flag numbers to the %eflags register
    registers[16] = carryFlag + zeroFlag + signFlag + overflowFlag;
    break;
  case je:
  ...
  case call:
    ...
    *((int*)(&memory[registers[6]])) = program_counter + 4;
    ...
    break;
  case ret:
    // If %esp is 1024, exit the program immediately
    if(registers[6] == 1024)
      {
	exit(0);
      }
